...
public class OrderDrawing implements Serializable{
	...
	*/
	...
	public void drawHelp(Graphics2D g2){
		...
		if(isCausalOrder && isTotalOrder && totalOrder != null && causalOrder != null){
			...
			if(fifoOrder != null){
				for(Interval inter: fifoOrder)
					draw(g2,inter.start,inter.end,fifoClockTable,Color.GREEN);
			}
		}
	...
	private Intersection intersect(Vector<Interval> causal,Vector<Interval> total){
		//el primer vector sera el de causal y el segundo el de total
		Intersection inter = new Intersection();
		int proc;
		//para cada elemento de vec 1 se comprueba si hay un intervalo en vec 2 y se calcula
		//la interseccion
		int startCausal;
		int startTotal;
		...
		for(Interval section1:causal){
			for(Interval section2:total){
				...
				if(proc == section2.start.process){	
					int start = Math.max(section1.start.tick, section2.start.tick);
					...
					if(section1.start.tick < section2.start.tick){
						...
						inter.causal.add(getInterval(proc,startCausal,finalCausal));
					}
					else{
						startCausal = section2.start.tick;
						finalCausal = section1.start.tick-1;
						inter.total.add(getInterval(proc,startCausal,finalCausal));
					}
					...
					if(section1.end.tick > section2.end.tick){
						...
						inter.causal.add(getInterval(proc,startTotal,finalTotal));
					}
					else{
						startTotal = section1.end.tick+1;
						finalTotal = section2.end.tick;
						inter.total.add(getInterval(proc,startTotal,finalTotal));
					}
