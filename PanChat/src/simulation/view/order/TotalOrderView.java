...
public class TotalOrderView implements OrderI,Serializable{
		...
		private boolean addLogicalOrder(CellPosition origin, CellPosition position) {
			boolean correctness = true; 
			// la correcion solo debe llevarse a cabo si es un mensajes de envio multiple
				...
				if(or == null){
					...
					//se aniade a la lista del origen, si ya se encontraba en la lista, debe quitar el valor
					//anterior
		...
		private boolean checkCorrectness(CellPosition origin,CellPosition position,int id){
			/*
			 * para comprobar la correccion se realiza la resta del tick position con todos
			 * los demas mensajes entregados por el proceso. El signo de esta resta tendrá 
			 * que corresponder con su respectivo mensaje en el resto de procesos
			 */
			boolean correctness = true;
			...
			/*por cada diferencia con cada uno de mis procesos se comprueba si la resta
			*coincide con los correspondientes mensajes en aquellos procesos que hayan
			*recibido el mensaje del cual se comprueba la correccion
			*/
			
			int i;
			//para todos los procesos
			wrong:
			for(CellPosition list:orVector.finalPos){
				i = list.process;
				if(i != position.process){
					//se busca el mensaje del que se busca correccion que OBLIGATORIAMENTE
					...
					//cuando se encuentra se calculan las diferencias con todos los demas
					//mensajes
					tmp = getDiference(i,total.finalPos.firstElement().tick);
						
					//para los mensajes comunes, debe haber acuerdo en el orden de entrega
					...
					while(it.hasNext()){
						...
						if(tmp.containsKey(difId)){
							...
							if((difValue>0 && value <0) || (difValue<0 && value>0)){
								correctness = false;
								break wrong;
							}
						}
		...
		public void getHelp(CellPosition origin,CellPosition finalPos){
			TotalMessage orVector = (TotalMessage) clockTable.get(origin);
			//indican el orden del mensaje a insertar con respecto a los demas mensajes
			...
			for(CellPosition vector:orVector.finalPos){		
				...
				if(vector.process != finalPos.process){
					...
					//todos los mensajes del vector que se entreguen antes se introducen
				...
				if(visited[proc]==false){
					...
					for(VectorI procVector:messages.get(proc)){
						...
						if(before.contains(actual.id))
							deliver.start.tick = Math.max(deliver.start.tick, actualFinal.tick +1);
						else if(after.contains(actual.id))
							deliver.end.tick = Math.min(deliver.end.tick, actualFinal.tick-1);
					}
