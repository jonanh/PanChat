...
public class FifoOrderView implements Serializable, OrderI {
	...
	private boolean addLogicalOrder(CellPosition origin, CellPosition position,
		...
		if (newVector.isOrigin == false)
			correctness = newVector.isCorrect((VectorClock) clockTable
					.get(origin));
		...
		if (correctness == true) {
			...
			if (newVector.isOrigin == false) {
				if (newVector.origin.tick < lastTick) {
					...
					lastTick = Math.max(lastTick, newVector.finalPos
							.firstElement().tick);
			 ...
			 * misma si no tiene flechas finales hay que eliminar de la lista de
			 * posiciones final de origen la que corresponde a la flecha erronea
			 */
			VectorClock originalClock = (VectorClock) clockTable
					.get(newVector.origin);
		...
		for (int i = originalTick; i <= lastTick; i++) {
			...
			for (int j = 0; j < simulationModel.getNumProcesses(); j++) {
				...
				if (actualVector != null
						&& actualVector.isOrigin == originOrFinal) {
	...
	public void getHelp(CellPosition origin, VectorClock newVector) {
		...
		// hay que encontrar el siguiente vector con el que se entra en
		// conflicto
		or = origin.clone();
		or.tick++;
		...
		// si nos hemos pasado de los tick maximos es porque no se ha encontrado
		// ninguno. Eso quiere decir
		// que hemos entrado en conflicto con una flecha de tick inferior
		arrowOrigin = origin.clone();
		if (or.tick <= lastTick) {
			// hay que encontrar la posicion final que corresponda al proceso
			// destino
		...
		if (or.tick > lastTick
				|| newVector.finalPos.firstElement().tick < end.tick) {
